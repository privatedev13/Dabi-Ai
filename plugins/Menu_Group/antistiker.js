...
module.exports = {
  name: 'antistiker',
  command: ['antis', 'antistiker'],
  tags: 'Group Menu',
  desc: 'Fitur anti stiker grup',
  prefix: true,
  ...
  run: async (conn, message, {
    ...
    groupData.gbFilter.stiker.antistiker = args[0].toLowerCase() === 'on';

    fs.writeFileSync(dbPath, JSON.stringify(db, null, 2));

    return conn.sendMessage(chatId, {
      text: `✅ Fitur anti stiker berhasil di-${args[0].toLowerCase() === 'on' ? 'aktifkan' : 'nonaktifkan'}.`
    }, { quoted: message });
  }
};